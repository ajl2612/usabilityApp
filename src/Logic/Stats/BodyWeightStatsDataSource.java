...
public class BodyWeightStatsDataSource {
  ...
  private String[] allColumns = { 
		  BodyWeightStatsEntry.BW_STATS_ENTRY_ID,
		  BodyWeightStatsEntry.BW_EXERCISE_ID,
		  BodyWeightStatsEntry.BW_REPS
  };
  ...
  public long addStat(int exerciseId, int reps) {
    ...
    Cursor cursor = database.query(BodyWeightStatsEntry.TABLE_NAME,
        allColumns, BodyWeightStatsEntry.BW_STATS_ENTRY_ID + " = " + insertId, null,
        null, null, null);
 ...
 public ArrayList<Pair<Integer, Integer>> getExerciseStats(int exerciseId){
	 	...
	 	Cursor cursor = database.query(BodyWeightStatsEntry.TABLE_NAME,
	        allColumns, BodyWeightStatsEntry.BW_EXERCISE_ID + " = " + exerciseId  , null, null, null, null);
	    cursor.moveToFirst();
	    
	    while (!cursor.isAfterLast()) {
	      pair = new Pair<Integer,Integer>(cursor.getInt(1), cursor.getInt(2));
	      dataPoints.add(pair);
	      cursor.moveToNext();
	    } 
	    return dataPoints; 
